...
- statistical significance — usually represented as –log₁₀(adjusted p-value)

By combining these two axes, volcano plots highlight genes that show both large expression changes and strong statistical support, making it easy to identify biologically meaningful up‑ and down‑regulated genes within each cell type.

## 🔍 Interpretation Guidelines

**Up-regulated genes (red-right side)**

Genes plotted in red on the right side represent transcriptional programs increased or activated in the condition for that specific cell type.

- log2FC > 0 (higher expression in Treatment vs Control)
- adjusted p-value ≤ α (statistically significant)
...
Genes plotted in red on the left side represent transcriptional programs decreased or inactivated in the condition for that specific cell type.
...
- not significant at the chosen adjusted p-value threshold (p_adj > α), and/or
...
A volcano plot provides a fast and intuitive overview of differential‑expression (DE) patterns within each cell type.

### Color Guide

| **Color** | **Meaning**              | **Criteria**                                       |   **Volcano side**                                              |
|-----------|--------------------------|----------------------------------------------------|-----------------------------------------------------------------|
| **Red**   | Up‑regulated             | log2FC > 0 & adjusted p-value ≤ α                  |   Right side (positive log₂FC)                                  |
| **Red**   | Down‑regulated           | log2FC < 0 & adjusted p-value ≤ α                  |   Left side (negative log₂FC)                                   |
| **Blue**  | Significant p-value only | adjusted p‑value ≤ α but log₂FC < threshold        |   log₂FC; Both sides (near 0 log₂FC)                            |
| **Green** | High fold‑change only    | log₂FC ≥ threshold but adjusted p‑value > α        |   Left = large negative FC; Right = large positive FC           |
| **Grey**  | Not significant          | adjusted p-value > α and log₂FC < threshold        |   Center region around log₂FC ≈ 0                               |

This visualization helps identify genes that show both strong statistical significance and biologically meaningful fold‑change in a cell‑type–specific manner.
...
The analysis focuses on the top `r n_value` genes ranked by significance to highlight the most biologically relevant up‑ and down‑regulated features.
...
if (!is.null(condition_value2)) {
  ...
  volcano_plot_results <- crete_volcano_plot(all.markers = all.markers_condition_value2, n_value = n_value, plots_dir = plots_dir_condition2)
}
```
...
if (!is.null(condition_value3)) {
  ...
  volcano_plot_results <- crete_volcano_plot(all.markers = all.markers_condition_value3, n_value = n_value, plots_dir = plots_dir_condition3)
}
```
