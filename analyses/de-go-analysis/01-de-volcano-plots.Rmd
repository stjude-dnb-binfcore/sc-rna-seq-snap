---
title: "Differential expression (DE) Volcano plots Per Cell Type for sc-RNA-Seq Analysis in 10X Genomics data"
...
- (a) calculates differential expression (DE) results for each cell type, and
- (b) generates volcano plots to summarize DE results for each cell type.
...
stopifnot(group_col %in% names(markers_df))


by_grp <- split(markers_df, markers_df[[group_col]])
...
for (cl in names(by_grp)) {
  ...
  up <- df %>%
    dplyr::filter(avg_log2FC > 0.25) %>% dplyr::arrange(p_val_adj) %>%
    slice_head(n = topN)
 
  dn <- df %>%
    dplyr::filter(avg_log2FC < 0.25) %>% dplyr::arrange(avg_log2FC) %>% dplyr::arrange(p_val_adj) %>%
    slice_head(n = topN)
  ...
  p_grp <- print(EnhancedVolcano::EnhancedVolcano(
    ...
    drawConnectors = TRUE,
    # If labels still parse as math, disable parsing:
    parseLabels    = FALSE,
    # Make significant points red only; others black
