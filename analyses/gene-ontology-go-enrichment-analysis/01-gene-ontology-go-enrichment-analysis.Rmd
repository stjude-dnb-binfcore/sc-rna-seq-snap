...
#markers_dir_module <- file.path(root_dir, "analyses", "cluster-cell-calling", "results", "02_find_markers")

# Input files
data_file <- file.path(data_dir, "seurat_obj_cell_types_annotations_all.rds")
#markers_file <- file.path(markers_dir_module, glue::glue("Res_{resolution_list}_Markers_top_{n_value}.tsv"))

# Create results_dir
results_dir <- file.path(module_results_dir, "01-gene-ontology-go-enrichment-analysis")
if (!dir.exists(results_dir)) {
  dir.create(results_dir)}

# Create plots directory
go_plots_dir <- file.path(module_plots_dir, "01-gene-ontology-go-enrichment-analysis") 
if (!dir.exists(go_plots_dir)) {
  dir.create(go_plots_dir)}

# Source functions
source(paste0(analysis_dir, "/util/run-clusterwise-go-enrichment-markers.R"))
```
...
```{r read-markers-cluster, echo=TRUE}
#markers_df <- read.csv(markers_file, sep = "\t", header = TRUE)
#head(markers_df)
```
