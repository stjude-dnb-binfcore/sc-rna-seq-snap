...
```{r align-assays, echo=TRUE}
# If assay names differ, rename the reference assay to match the query
if (reference_assay != assay) {

  message(
    sprintf(
      "Renaming reference assay '%s' -> '%s' to match query.",
      reference_assay, assay
    )
  )

  reference_obj <- RenameAssays(
    object = reference_obj,
    old.names = reference_assay,
    new.names = assay
  )

  # Ensure defaults are synced after renaming
  DefaultAssay(reference_obj) <- assay
}
```
